...
interface Props {
  ...
  onDelete: (customer: CustomerList) => void
  canEdit?: boolean
  canDelete?: boolean
}
...
export default function CustomersTable({
  ...
  const columns = [
    ...
    columnHelper.accessor('customer_type', {
      header: 'Tipo',
      cell: (info) => (
        <Badge variant="outline">{CUSTOMER_TYPE_LABEL[info.getValue()] ?? info.getValue()}</Badge>
      ),
    }),
  ...
  if (!data.length) return <div className="py-12 text-center text-muted-foreground">No hay clientes registrados.</div>
