...
export default function UsersTable({ data, isLoading, page, hasNextPage, onPageChange, onEdit, onDelete }: Props) {
  const columns = [
    ...
    columnHelper.accessor('is_superuser', {
      header: 'Superadmin',
      cell: (info) =>
        info.getValue() ? <Badge variant="destructive">Sí</Badge> : null,
    }),
    columnHelper.display({
      id: 'actions',
      header: 'Acciones',
  ...
  if (!data.length) return <div className="py-12 text-center text-muted-foreground">No hay usuarios registrados.</div>
