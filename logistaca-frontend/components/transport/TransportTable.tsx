...
const columnHelper = createColumnHelper<TransportList>()
...
export default function TransportTable({
  ...
  const columns = [
    ...
    columnHelper.accessor('license_plate', { header: 'Placa' }),
    columnHelper.accessor('vehicle_type', {
      header: 'Tipo',
      cell: (info) => (
        <Badge variant="outline">{VEHICLE_TYPE_LABEL[info.getValue()] ?? info.getValue()}</Badge>
      ),
    }),
  ...
  if (!data.length) return <div className="py-12 text-center text-muted-foreground">No hay vehículos registrados.</div>
